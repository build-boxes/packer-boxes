...
source "virtualbox-iso" "autogenerated_1" {
  ...
  #boot_command            = ["<esc><wait>", "install <wait>", " preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg <wait>", "debian-installer=en_US.UTF-8 <wait>", "auto <wait>", "languagechooser/language-name=English <wait>", "countrychooser/shortlist=US <wait>", "locale=en_US.UTF-8 <wait>", "kbd-chooser/method=us <wait>", "keyboard-configuration/xkb-keymap=us <wait>", "netcfg/get_hostname={{ .Name }} <wait>", "netcfg/get_domain=vagrantup.com <wait>", "fb=false <wait>", "debconf/frontend=noninteractive <wait>", "console-setup/ask_detect=false <wait>", "console-keymaps-at/keymap=us <wait>", "grub-installer/bootdev=/dev/sda <wait>", "<enter><wait>"]
  boot_wait               = "5s"
  disk_size               = 28672
  ...
  vboxmanage              = [["modifyvm", "{{ .Name }}", "--memory", "2048"], ["modifyvm", "{{ .Name }}", "--cpus", "1"] 
                              ...
                              #                             , ["storagectl", "{{ .Name }}", "--name", "SATA Controller", "--add", "sata", "--controller", "IntelAhci", "--portcount", "1", "--bootable", "on"],
                              #                             ["storagectl", "{{ .Name }}", "--name", "IDE Controller", "--add", "ide", "--controller", "PIIX4"] ]
  virtualbox_version_file = ".vbox_version"
  vm_name                 = "packer-debian-12-efi-amd64"
}
...
build {
  ...
  post-processors {
    post-processor "vagrant" {
      output = "builds/{{ .Provider }}-debian12efi.box"
    }
    post-processor "vagrant-cloud" {
      box_tag = "raufhammad/debian12efi"
      version = "${var.version}"
    }
  }
}
