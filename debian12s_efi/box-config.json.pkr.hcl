...
source "virtualbox-iso" "autogenerated_1" {
  ...
  vboxmanage              = [["modifyvm", "{{ .Name }}", "--memory", "1024"], ["modifyvm", "{{ .Name }}", "--cpus", "1"], ["modifyvm", "{{ .Name }}", "--firmware", "efi64"]]
...
build {
  ...
  post-processors {
    post-processor "vagrant" {
      output = "builds/{{ .Provider }}-debian12s_efi.box"
    }
    post-processor "vagrant-cloud" {
      box_tag = "raufhammad/debian12s_efi"
      version = "${var.version}"
    }
  }
}
